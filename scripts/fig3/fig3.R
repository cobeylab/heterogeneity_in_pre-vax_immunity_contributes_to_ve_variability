.pkgs <- c("tidyverse", "cowplot", "here", "paletteer", "RcppTOML", "optparse")
...
# source(here("exp", "abm_model", "scripts", "clean_sim_data.R"))
# source(here("exp", "waning_vaccine_odes", "scripts", "waning_vax_imm_ode.R"))
...
# suscep_ratios <- suscep_dt %>%
#     select(year, vax_status, avg_suscep) %>%
#     distinct() %>%
#     pivot_wider(names_from = vax_status, values_from = avg_suscep) %>%
#     mutate(suscep_ratio = signif(Vaccinated / Unvaccinated, digits = 3))
...
#   filter(year >= 13) %>%
  ...
  rowwise() %>%
  mutate(
    # T_tot = ifelse(vax_imm_halflife == 0.0,
    #     total_vax_factor(time, 1 - vax_efficacy, Inf),
    #     total_vax_factor(time, 1 - vax_efficacy, vax_imm_halflife)),
    waning_label = ifelse(vax_imm_halflife == 0.0, 
        "Constant vaccine protection",
        "Waning vaccine protection")
  ) %>%
  ungroup() %>%
#   mutate(avg_eff = (1 - (T_tot / time)) * 100) %>%
#   select(-c(T_tot, vax_imm_halflife))
...
final_ve_plt <- ggplot(ve_dt) +
    ...
    #   x = 3,
    #   y = 53,
    #   label = "True vaccine efficacy = 50%",
    #   xoff = 1.1,
    #   yoff = 2
    # ) +
...
suscep_plt <- ggplot(suscep_dt) +
    ...
    # geom_label(
    #   data = suscep_ratios,
    #   aes(x = year, y = 2.75, label = suscep_ratio),
    #   inherit.aes = FALSE
    # ) +
